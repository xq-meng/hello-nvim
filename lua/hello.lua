...
function hello.align_center(tbl, state)
...
function hello.pad_margin(tbl, state, margin, shrink)
    local longest = longest_line(tbl)
    local left
    if shrink then
...
function hello.highlight(state, end_ln, hl, left)
...
function hello.resolve(to, el, opts, state)
...
function layout_element.text(el, conf, state)
    if type(el.val) == "table" then
        ...
        if conf.opts and conf.opts.margin and el.opts and (el.opts.position ~= "center") then
            local left
            val, left = hello.pad_margin(val, state, conf.opts.margin, if_nil(el.opts.shrink_margin, true))
            padding.left = padding.left + left
        end
        if el.opts then
            if el.opts.position == "center" then
                val, _ = hello.align_center(val, state)
            end
        ...
        if el.opts and el.opts.hl then
            hl = hello.highlight(state, end_ln, el.opts.hl, padding.left)
        end
        state.line = end_ln
        return val, hl
    end
    ...
    if type(el.val) == "function" then
        return hello.resolve(layout_element.text, el, conf, state)
    end
end
...
function layout_element.button(el, conf, state)
    ...
    if el.opts then
        if el.opts.position == "center" then
            local left
            val, left = hello.align_center(val, state)
            if el.opts.align_shortcut == "right" then
                padding.center = padding.center + left
            end
            padding.left = padding.left + left
    ...
    if el.opts and el.opts.hl_shortcut then
        ...
        if el.opts.align_shortcut == "right" then
            hl = hello.highlight(state, state.line, hl, #el.val + padding.center)
        else
            hl = hello.highlight(state, state.line, hl, padding.left)
        end
    end
    ...
    if el.opts and el.opts.hl then
        ...
        list_extend(hl, hello.highlight(state, state.line, el.opts.hl, left))
    end
...
function layout_element.group(el, conf, state)
    if type(el.val) == "function" then
        return hello.resolve(layout_element.group, el, conf, state)
    end
...
function keymaps_element.group(el, conf, state)
    if type(el.val) == "function" then
        hello.resolve(keymaps_element.group, el, conf, state)
    end
...
local function enable_alpha(conf)
    ...
    vim.cmd(noautocmd ..
    [[  silent! setlocal bufhidden=wipe nobuflisted colorcolumn= foldlevel=999 foldcolumn=0 matchpairs= nocursorcolumn nocursorline nolist nonumber norelativenumber nospell noswapfile signcolumn=no synmaxcol& buftype=nofile filetype=hello nowrap

        augroup alpha_temp
        au!
        autocmd BufUnload <buffer> lua require('hello').close()
        autocmd CursorMoved <buffer> lua require('hello').move_cursor()
        augroup END
    ]])
    ...
    if conf.opts then
        if if_nil(conf.opts.redraw_on_resize, true) then
            vim.cmd([[
                autocmd alpha_temp VimResized * lua require('hello').redraw()
                autocmd alpha_temp BufLeave,WinEnter,WinNew,WinClosed * lua require('hello').redraw()
            ]])
        end
...
function hello.start(on_vimenter, conf)
    local window = vim.api.nvim_get_current_win()

    hello.move_cursor = function()
    ...
    local function draw()
        ...
        vim.api.nvim_buf_set_keymap(
            ...
            "n",
            "<CR>",
            "<cmd>lua require('hello').press()<CR>",
        ...
        vim.api.nvim_buf_set_keymap(
            ...
            "n",
            "<M-CR>",
            "<cmd>lua require('hello').queue_press()<CR>",
            { noremap = false, silent = true }
        )
...
function hello.setup(config)
    ...
    vim.cmd([[
        command! Alpha lua require'hello'.start(false)
        command! AlphaRedraw lua require('hello').redraw()
        augroup alpha_start
        au!
        autocmd VimEnter * nested lua require'hello'.start(true)
        augroup END
    ]])
end

hello.layout_element = layout_element
hello.keymaps_element = keymaps_element
